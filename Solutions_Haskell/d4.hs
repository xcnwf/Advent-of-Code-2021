...
parse_bingo_board_lines lines board i = 
    ...
    else let nb = (map (\x -> (x, False))  (map (read :: String -> Int) (words (head lines))))
...
bingo_check board = 
    ...
    in if line_check then Right(board) 
       else if any (==True) (foldl (\acc line -> zipWith (&&) (map snd line) acc) (repeat True) board) 
            then Right(board)
            else Left(board)
